    ...
    public function doAc(){
        ...
        if($this->form_validation->run('login') === FALSE){
            ...
            if(isset($admin['password']) && checkPassword($password, $admin['password'])){
                $this->input->set_cookie('uid',$admin['user_name'],3600*24*6,'','/','','',TRUE);
                $this->input->set_cookie('ukey', UKEY,3600*24*6,'','/','','',TRUE);
                $this->input->set_cookie('pkey', md5(PKEY . UKEY . $admin['user_name']),3600*24*6,'','/','','',TRUE);
                ajaxJson('登录成功');
            }else{
